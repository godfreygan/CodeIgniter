<?php
    ...
    if ( !class_exists ( 'ezSQLcore' ) ) {
        die( '<b>Fatal Error:</b> ezSQL requires ezSQLcore (application/helpers/ez_sql_core_helper.php) to be included/loaded before it can be used' );
    }
    ...
    class ezSQL_codeigniter extends ezSQLcore {
        ...
        function escape ( $str, $like = false ) {
            return $this->CI->db->escape_str ( stripslashes ( $str ), $like );
        }

        function filter_value ( $str, $like = false ) {
            return "'" . $this->escape ( $str, $like ) . "'";
        }
