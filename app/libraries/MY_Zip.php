...
class MY_Zip
{
	...
	public function visitFile($path)
	{
		...
		while(($file = $fdir->read()) !== false)
		{
			...
			$this->filearr[] = is_dir($pathSub) ? $pathSub."/" : $pathSub;
	...
	public function Zip($dir, $saveName)
	{
		...
		@ob_end_clean();
		$this->visitFile($dir);
		if(count($this->filearr) == 0){ return; }
	...
	// ------------------------------------------------------ //
	public function ZipDown($dir,$name)	
	{
		if(@!function_exists('gzcompress')){ return; }
	...
	// $archive->ZipAndDownload("需压缩的文件所在目录");
	// ------------------------------------------------------ //
	public function ZipContent($path,$k=0,$start_time=0,$end_time=0)	
	{
		//if(@!function_exists('gzcompress')){ return; } 
		@ob_end_clean();		
		...
		foreach($this->filearr as $key=>$file)
		{
			...
			{ 	 
				if($key >= $k)
				{
					if($totalsize < 1024*50000)
					{
						//echo $file.'====='.$key.'----'.$totalsize.'<br>';
						if(!file_exists($file) || !is_file($file)){ continue; }
	...
	// $archive->ZipAndDownload("需压缩的文件所在目录");
	// ------------------------------------------------------ //
	public function ZipContent2($path)	
	{
		//if(@!function_exists('gzcompress')){ return; } 
		@ob_end_clean();
		$this->filearr=array();	
	...
	// $archive->ZipAndDownload("需压缩的文件所在目录");
	// ------------------------------------------------------ //
	public function ZipDown2($dir,$name,$stime,$etime)	
	{
		if(@!function_exists('gzcompress')){ return; }
